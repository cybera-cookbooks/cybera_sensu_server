...
monit_check 'uchiwa' do
  check "if does not exist for 3 cycles then restart 
    if does not exist for 3 cycles then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup 
      else if succeeded then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup"
...
monit_check 'sensu-api' do
  check "if does not exist then restart
    if does not exist then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup 
      else if succeeded then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup"
...
monit_check 'influxdb' do
  check_type 'host'
  check "if failed port 8086 for 5 cycles then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup 
    else if succeeded then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup"
...
monit_check 'elasticsearch-health' do
  check_type 'host'
  check "if failed url http://elk-client.edu.cybera.ca:9200/_cluster/health and content =='green'
    with timeout 60 seconds then exec  #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup
    else if succeeded then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup"
...
monit_check 'rabbitmq_queue_monitor' do
  check_type 'program'
  with "path \"#{node[:monit][:plugin][:rabbitmq][:queue_monitor]} 500\" as uid rabbitmq "
  check "if status != 0 for 5 cycles then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup
    else if succeeded then exec #{node[:monit][:alert][:slack][:script_file]} as uid nobody and gid nogroup"
end
